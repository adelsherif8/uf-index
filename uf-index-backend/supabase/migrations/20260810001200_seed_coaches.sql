...
-- Creating coaches.
--
-- A coach is an auth user with a row in `coaches`, so the person must sign up
-- in the app first — there is no way to create a coach who cannot log in.
...
begin
  ...
  if uid is null then
    raise exception
      'No account for %. They need to sign up in the app first — a coach must be able to log in.',
      p_email;
  end if;
...
-- not exist yet. Re-run the call by hand once it does.
do $$
begin
  perform public.ensure_coach('adelsherif8@gmail.com', 'Adel Emad', true);
  raise notice 'Admin coach set: Adel Emad';
exception when others then
  raise notice 'Admin coach not set (%). Sign up in the app, then run: select public.ensure_coach(''adelsherif8@gmail.com'', ''Adel Emad'', true);', sqlerrm;
end $$;
